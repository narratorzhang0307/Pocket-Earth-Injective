...
export function buildPocketPodcast({ date, bundles = [], generatedAt = new Date().toISOString() } = {}) {
  ...
  const anchoredRoots = [...new Set(selected.filter((item) => item.edition?.anchor?.txHash).map((item) => item.edition.editionRoot))]
  ...
  run.push(event(6, 'receipt', 'complete', { podcastId, editionRoots, anchoredRoots, automaticChainWrite: false }))
  ...
  return {
    ...
    memory: {
      ...
      editionRoots,
      anchoredRoots,
      policy: '完整新闻保留七天；通过复核的记录与版次根进入长期精选层。',
    },
    run: {
