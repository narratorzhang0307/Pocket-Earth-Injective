...
export function compactReviewedEdition(proof) {
  ...
  return {
    ...
    state: 'reviewed-and-anchored',
    memoryTier: 'L3-long-term-reviewed-memory',
    network: String(proof.network || 'Injective EVM Testnet'),
    chainId: Number(proof.chainId),
    publisher: String(proof.publisher || ''),
    contractAddress: String(proof.contractAddress || ''),
    transactionHash: assertHash(proof.transactionHash, 'transaction_hash'),
    scanUrl: String(proof.scanUrl || ''),
    committedAt: String(proof.committedAt || ''),
    date: proof.date,
    ...
    retentionPolicy: {
      lifetime: 'permanent',
      includes: 'reviewed claims, final sources, Merkle proofs and Injective anchor',
