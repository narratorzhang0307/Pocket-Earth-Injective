...
| `/home/pi/earth-answers` | Earth Answers | Reviewed 365-entry annual action-guidance edition | Persistent; content is separate from executable launcher code |
| `/etc/pocket-earth-edge.env` | root | Feed URL and device bearer token | Persistent, mode `0600`, never committed or logged |
| `/var/lib/pocket-earth-edge` | Pocket Earth service | Durable feed cursor and Earth Answer reveal history | Persistent across reboot |
...
This uses a project-first layout for source and device code: Sunset Radio,
Pocket Earth, and the Earth Answers content edition are parallel project trees
under `/home/pi`. It still follows the
...
on this personal device, also the clear parent for the independently maintained
capability trees. Trusted Pocket Earth sub-agents are parallel branches
...
| `pocket-earth-launcher.service` | `口袋地球` launcher for music, knowledge podcast/reading, and action guidance; owns no feed, wallet, or radio business logic |
