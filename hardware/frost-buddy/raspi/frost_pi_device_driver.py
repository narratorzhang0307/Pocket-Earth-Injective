...
contract. It never reads a wallet, Pocket Earth profile, or precise location.
...
INK = (10, 10, 12)
PAPER = (245, 244, 239)
INJECTIVE_GREEN = (0, 244, 139)
CHAIN_MAGENTA = (255, 20, 199)
...
def render_evidence_card(actions: list[dict]) -> Image.Image:
    ...
    accent = MUSIC_BLUE if source_kind == "music_now_playing" else INJECTIVE_GREEN
    ...
    draw.text((166, 12), "INJECTIVE", font=_font(9, "mono"), fill=INK)

    kind_label = "MUSIC AGENT" if source_kind == "music_now_playing" else "LIVE CHAIN DISPATCH"
...
p{{color:#00f48b;font-weight:700}}</style><header>POCKET EARTH ON INJECTIVE</header>
...
def _demo_actions() -> list[dict]:
    ...
    return [
        {"type": "state", "state": "attention", "sourceKind": "chain_dispatch", "createdAt": now},
        {"type": "tts", "text": "Pocket Earth 已连接 Injective。Frost 把链上见闻带回房间了。"},
        {
            "type": "display",
            "title": "链上见闻已抵达",
            "body": "实时读回 Pocket Earth agent 身份，公开证据可以独立复验。",
