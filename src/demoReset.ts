...
(function demoReset() {
  ...
  try {
    ...
    if (typeof localStorage !== 'undefined') {
      ...
      // ?demo：清完后注入一份示例画像，让广场 / 各 agent 一进去就有内容（录 demo 省去现场积累；与链上 agent 口味有交集）
      if (typeof location !== 'undefined' && /[?&]demo\b/.test(location.search)) {
        localStorage.setItem('pe.profile.v1', JSON.stringify({
          domains: {
            books: { genre: [{ tag: '拉美文学', n: 7 }, { tag: '魔幻现实', n: 5 }, { tag: '马尔克斯', n: 4 }] },
            movies: { genre: [{ tag: '黑色电影', n: 6 }, { tag: '老电影', n: 4 }, { tag: '希区柯克', n: 3 }] },
            music: { genre: [{ tag: '爵士', n: 5 }, { tag: '蓝调', n: 3 }, { tag: '即兴', n: 2 }] },
            travel: { place: [{ tag: '北欧', n: 4 }, { tag: '京都', n: 3 }, { tag: '冰岛', n: 2 }] },
          }, seedVersion: 1, updatedAt: new Date().toISOString(),
        }));
      }
    }
