...
function chainAgentToNeighbor(a: { agentId?: unknown; identityTuple?: string; name?: string; scanUrl?: string; card?: { name?: string; description?: string; tags?: string[] } }, myTags: string[]): Neighbor {
  ...
  const common = tags.filter((t) => mine.has(t));
  // 相似度：有口味交集越多越高；无交集 / 画像薄时按 agent 标签稳定抖动，避免一排都同分难看
  const overlap = myTags.length && tags.length ? common.length / Math.min(tags.length, myTags.length) : 0;
  const sim = Math.max(55, Math.min(99, Math.round(55 + overlap * 44 + (hash(tags.join('·')) % 16))));
  return {
    tag: common[0] || tags[0] || '链上身份',
    field: 'agent',
...
// 链上 agent 无地理坐标 → 用 agentId 确定性散布到全球（每个 agent 固定一处、不随刷新跳）。
function agentGeo(seed: string): { lat: number; lng: number } {
  const h = hash(seed || 'agent');
  return { lat: ((h % 12000) / 100) - 60, lng: (((h >> 7) % 36000) / 100) - 180 };
}
...
export default function PublicPlazaPage({ onBack }: Props) {
  ...
  // P0-6：从 Injective 链上读真实 agent（空 / 失败回落本机示意邻居，不白屏）
  ...
  const { neighbors: demoNeighbors, myTags } = useMemo(() => {
    const p = getProfile();
  ...
  useEffect(() => {
    ...
    fetch('/api/injective?tool=list-agents&builderCode=pocket-earth&limit=5&top=47')
      ...
      .then((d) => {
        ...
        const mapped = d.agents.map((a: Parameters<typeof chainAgentToNeighbor>[0]) => chainAgentToNeighbor(a, myTags)).sort((x: Neighbor, y: Neighbor) => y.sim - x.sim);
        setChainNeighbors(mapped);
        // P0-7：把链上 agent 钉到地球（确定性散布坐标；markPlace 内部按 prefix+key 去重，不重钉）
        for (const n of mapped) {
          if (n.agentId) markPlace({ kind: 'agent', prefix: 'uag-', key: n.agentId, label: n.alias, geo: agentGeo(n.agentId), meta: { scanUrl: n.scanUrl, sim: n.sim, onChain: true } });
        }
      })
      .catch(() => {});
    return () => { alive = false; };
  ...
  // 链上有真实 agent 就用真实的，否则回落本机示意邻居（架构就绪，等 testnet 有 agent / 自己注册后自动填充）
  const neighbors = chainNeighbors.length ? chainNeighbors : demoNeighbors;
  ...
  useEffect(() => {
    ...
    const onChain = report.some((n) => n.onChain);   // 链上模式：遇见的是 Injective 链上注册的真实 agent
    const list = report.map((n) => `${n.alias}（${FIELD_LABEL[n.field] || n.field}「${n.tag}」）`).join('；');
    const prompt =
      `你是用户的社交特使 FROST，今天替 ta 去${onChain ? ' Injective 链上的 agent 广场（这些是在链上注册了身份的真实 agent）' : ' agent 公共广场'}社交。ta 的口味：${taste || myTags.join('、')}。\n` +
    ...
    (async () => {
      ...
      if (!recap) recap = onChain
        ? `今晚我去 Injective 链上的广场转了一圈，遇见 ${neighbors.length} 个注册了链上身份的 agent，和其中 ${report.length} 个口味相投。`
        : `今晚我在广场转了一圈，遇见 ${neighbors.length} 个人，和其中 ${report.length} 个聊得来。`;
