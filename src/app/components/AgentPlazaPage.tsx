...
// 前瞻例（观鸟/播客/链上见闻）的 INSTALL 只切本地态、不真装载远程代码（PWA 跑第三方代码=安全黑洞），
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}
      ...
      <div className="flex items-center gap-2">
        ...
        <button type="button" onClick={badge.onClick} disabled={!badge.onClick}
...
export default function AgentPlazaPage({ onBack, onRun }: Props) {
  ...
  // 状态条是广场总况（不随筛选变）：已装=自带真 agent + 已添加的前瞻例；可装=前瞻例里还没添加的。
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2.5 space-y-2.5">
        ...
        <div className="border-2 border-black bg-white p-3 shadow-[2px_2px_0_rgba(0,0,0,0.85)]">
          <div className="text-[11px] text-black/70 leading-snug">
            每个人都能创建和发布空间 agent；上架前必须说清：处理哪类<b>空间对象</b> · 需要哪些<b>权限</b> · 跑在<b>端侧/云端</b> · 是否关联 <b>Injective 链上身份</b>。
          </div>
          <div className="mt-2 flex flex-wrap gap-1">
        ...
        <button type="button" onClick={() => onRun('agent-forge')}
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-[10px] text-black/60 leading-snug mt-0.5">符合空间逻辑才能上架：声明空间对象 · 申请最小权限 · 选端/云 · 可选择关联 Injective 身份 → 过审分发</div>
          </div>
        ...
        <div className="grid grid-cols-4 gap-1.5">
          {FILTERS.map((f) => (
            <button type="button" key={f.key} onClick={() => setFilter(f.key)} aria-pressed={filter === f.key}
