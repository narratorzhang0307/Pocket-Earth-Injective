...
// 'agent' = Injective 链上 agent 身份（公共广场上遇见的、口味相近的其他 agent，钉到地球）。
export type MarkerKind = 'music' | 'photo' | 'movie' | 'book' | 'travel' | 'council' | 'custom' | 'agent';
...
export const MARKER_KINDS: { kind: MarkerKind; label: string; color: string }[] = [
  ...
  { kind: 'agent', label: '链上', color: '#7c5cff' },
];
export const KIND_COLOR: Record<MarkerKind, string> = { music: '#00ff88', photo: '#00e5ff', movie: '#ffb000', book: '#b388ff', travel: '#ff3b6b', council: '#caa64a', custom: '#ff8a3d', agent: '#7c5cff' };
...
export function ensureHeavyMarkers(): Promise<void> {
  ...
  heavyPromise = Promise.all([import('./movies'), import('./books')]).then(([mv, bk]) => {
    ...
    heavyLoaded = true;
  }).catch((e) => { heavyPromise = null; throw e; });   // 失败清缓存→下次切回地球入口重新 import；保持 reject 让上层 .catch 照常吞（否则一次瞬时 chunk 404/离线后电影·书标记整会话消失、不自愈）
  return heavyPromise;
}
