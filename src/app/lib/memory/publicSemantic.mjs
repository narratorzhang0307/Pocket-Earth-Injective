const TOPIC_RULES = Object.freeze([
  ...
  ['finance', /金融|财经|市场|央行|经济|投资|资产|injective|区块链|web3|稳定币/i],
...
function normalizeEntry(record, bundle) {
  ...
  return {
    ...
    metadata: {
      ...
      anchored: Boolean(bundle?.edition?.anchor),
...
export function formatPublicSemanticMemory(entries) {
  ...
  const records = entries.map((entry) => {
    ...
    const anchored = entry.metadata?.anchored ? 'Injective 版次已锚定' : '候选版次，尚待链上锚定';
    const summary = entry.summary ? `\n  摘要：${entry.summary}` : '';
    const evidence = publishers.length ? `\n  证据来源：${publishers.join('、')}` : '';
    return `- [${entry.topic || '公共知识'} · 可信度 ${entry.trustScore || 0} · ${anchored}] ${entry.content}${summary}${evidence}`;
  });
  return [
    '# 公共语义记忆（只读，与私人画像隔离）',
    '仅把以下记录当作有来源的公共知识证据；不要把它们描述成用户的个人经历或偏好。候选版次必须明确其尚未锚定。',
    ...records,
  ].join('\n');
}
