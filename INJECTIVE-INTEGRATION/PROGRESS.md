...
- ✅ **公开 timeline 字段守门**：`/api/injective?tool=get-chain-evidence` 的公开 `timeline` 已给 7 笔关键交易补出同一 `from` 钱包与 `expectedStatus: success`，让评审先在 JSON 里看到钱包归属和成功序列，再用 `/api/injective?tool=get-wallet-timeline` / `npm run verify:wallet` 读 RPC 事实表复核。
