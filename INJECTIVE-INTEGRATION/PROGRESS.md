...
- ✅ **链上证据脚本**：`verify-agent43.mjs` 可读回 `builderCode: pocket-earth` 并核验 #43、钱包页、握手合约页；`verify-handshake-contract.mjs` 可重编译 `SocialHandshake.sol` 并比对 Injective testnet 部署 runtime bytecode；`verify-handshake.mjs` 可解码真实握手事件（agentA 43 · agentB 44 · score 88）。
