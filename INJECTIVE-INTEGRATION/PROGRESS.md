...
- ✅ **链上证据脚本**：`verify-agent43.mjs` 可读回 `builderCode: pocket-earth` 并核验 #43、钱包页、握手合约页；`verify-handshake.mjs` 可解码真实握手事件（agentA 43 · agentB 44 · score 88）。
- ✅ **demo 预置画像**：`?demo` 参数清零后自动注入示例口味画像，录 demo 一进广场就丰满（`demoReset.ts`）。
- ✅ **交付物**：`.gitignore` 加固（私钥/第三方库不入库）、`.env.example` 补 INJ_ 说明、`DEMO-SCRIPT.md`（3 分钟分镜 + 链上验证凭证）、`register-fleet.mjs` / `verify-plaza*.mjs` / `verify-agent43.mjs` / `verify-handshake.mjs` 复现工具。
