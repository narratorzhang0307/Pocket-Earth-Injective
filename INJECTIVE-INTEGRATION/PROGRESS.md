...
- **PPT 硬件边界守门**：`verify:hardware` 现在会明确固定“事件桥、Pi adapter、Whisplay 小屏幕、RGB LED、本地 MiniMax / 离线 TTS、手机镜像与 systemd sidecar 已实现并真机验收；BLE / MQTT / serial 仍是可选 transport”这条边界，同时避免把 Frost Edge Node 写成已量产、已签名或硬件收入优先路径。
...
- `README.md`：对外总览，按空间知识库、frost-agent harness、Frost Passport、Profile Chain、Agent Plaza、Frost Edge Node 分层；`5.1 最终整合版对照与技术深挖` 把 PPT 页段映射到技术落点和复验入口，并补充作者方法来源只作为产品方法解释、链上证明仍以仓库/API/交易为准；`5.1.1 FROST Chronicle：画像演化史与三件交付` 把 PPT 收尾压成四个公开核验点：AI 分身公开画像的来源证明、现在能打开、公开 GitHub + 完整 README / Demo 视频 / Pitch Deck 三件交付、Built on Injective 的公共身份层 / 画像版本见证层 / 未来结算回执层；`5.3 商业路径与三条边界` 把商业判断收束为“不走纯社交变现 / 不走代币优先 / 不走重资本硬件路线”，并用 `marketLandscapeBoundary`、`commercialFlywheel`、`preferredPath`、`rejectedPaths` 把市场判断接入公开证据包。
...
- `INJECTIVE-INTEGRATION/DEMO-SCRIPT.md`：3 分钟以内录制脚本，镜头优先级是 `agentId 43`、钱包页、真实 SocialHandshake 事件字段、SocialHandshake creation/runtime bytecode、public-plaza、地球 agent 标记、Nightly Chain Dispatch、隐私边界；开头新增 `Pocket Earth 30 秒主线讲法`，把用户痛点、地球索引、Injective 公共见证、Frost Edge Node 和 Agent Plaza 商业边界压成录屏口播骨架。
- `INJECTIVE-INTEGRATION/PLAN.md` / `INJECTIVE-INTEGRATION/RESEARCH.md`：历史计划与研究材料，只解释方案来源、SDK 事实和后续边界；当前事实以 `README.md`、`INJECTIVE-INTEGRATION/CHAIN-EVIDENCE.md`、`INJECTIVE-INTEGRATION/JUDGE-QUICKSTART.md` 和五条只读 API 为准。P1/P2、可选付费、外部托管、硬件节点服务只能作为 future boundary / dry-run 槽位描述，不能覆盖当前 Blockscout、钱包时间线、data URI Agent Card 和公开证据包。
