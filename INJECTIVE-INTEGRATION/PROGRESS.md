...
- ✅ **链上证据脚本**：`verify-agent43.mjs` 可读回 `builderCode: pocket-earth` 并核验 #43、钱包页、握手合约页；`verify-handshake-contract.mjs` 可核验 SocialHandshake 部署者/部署时序，并重编译 `SocialHandshake.sol` 比对 Injective testnet runtime bytecode；`verify-handshake.mjs` 可解码真实握手事件（agentA 43 · agentB 44 · score 88）。
