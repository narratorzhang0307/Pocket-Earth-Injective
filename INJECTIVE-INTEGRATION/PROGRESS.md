...
- ✅ **Agent 证据行守门**：`/api/injective?tool=get-chain-evidence` 的每个 `agents[]` 行已直接带上 owner、`builderCode`、registry、registry 链接、mint from `0x0`、mint tx hash/block 和 mint tx 链接；`npm run verify:evidence`、`npm run verify:public-proof`、`npm run verify:public-apis` 和 `node INJECTIVE-INTEGRATION/verify-api-read-tools.mjs` 都会检查这些字段，评审不用在多个 JSON 区块之间来回拼证据。
