...
`/api/injective?tool=get-chain-evidence` also returns `reviewBrief`: a judge-facing summary with one line, four Injective proof cards, three contest-fit mappings, and a four-step review path. `reviewChecklist` then expands that into what each public proof is meant to verify. It also includes `competitionAlignment`, which maps the same public proofs to AI social agents, Injective execution, the physical Frost Buddy extension, and the privacy-first evidence boundary. `submissionLinks` keeps the final review entry points together: the GitHub repo, live `?demo` URL, public evidence API, fleet API, and wallet timeline API. `submissionChecklist` maps the contest submission requirements back to public proof: public GitHub + README, Injective integration, sub-3-minute demo script, pitch-deck notes, and read-only review APIs. `npm run verify:public-proof` treats this JSON as the judge-facing contract and guards its field shape, commands, public URLs, repo boundary, and secret/local-path leak surface.
