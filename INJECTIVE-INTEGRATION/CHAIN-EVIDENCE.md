...
| Product loop | `publicReadApis`, `hardwareBridge`, `hardwareBridge.marketBoundary`, `get-hardware-bridge-proof`, `plazaFlow`, `recordingOrder[].evidenceFocus` | public-plaza reads the chain; agent-plaza keeps the install loop separate; Frost Edge Node receives only public event handoffs and stays framed as a prototype / developer-kit endpoint |
