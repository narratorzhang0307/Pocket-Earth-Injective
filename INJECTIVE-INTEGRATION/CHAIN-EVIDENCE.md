...
For a 60-second review path, open `JUDGE-QUICKSTART.md` first. `/api/injective?tool=get-chain-evidence` also returns `agents[].proofApi` and `registryMintEvents`, a public event-level table for `agentId 43-47` with mint-from, owner, tx hash, block number, tx link, and agent identity link, plus `registryMintSummary` for the agentId range, same owner, zero-address mint source, first/last block, and local registry verification command. `/api/injective?tool=get-agent-proof&agentId=43` is the single-agent card for the main Frost identity. The same evidence package now exposes a wallet `timeline` where every key transaction carries `from: 0x6D5ABec67Ba6387691DB42c48Dd1DA736e1dC934` and `expectedStatus: success`, plus a `timelineSummary` that summarizes the owner, event count, first/last block, first/last timestamp, and RPC verification path. It also returns `handshakeProof`, a compact summary of the real `agentId 43 <-> 44` SocialHandshake transaction with score `88`, Blockscout links, timestamp, block number, public field list, and the local verification command, while keeping raw profile fields off-chain. Reviewers can see the same-wallet success sequence and the handshake meaning before opening the RPC-backed timeline. It then returns `reviewBrief`: a judge-facing summary with one line, four Injective proof cards, three contest-fit mappings, and a four-step review path. `judgeRunbook` compresses the live review into five steps: agent identity, owner wallet, public evidence API, public read API suite, and `npm run verify:demo`. `reviewChecklist` expands that into what each public proof is meant to verify, including a dedicated `registry-mint-events` checkpoint for ERC-8004 mint provenance. It also includes `competitionAlignment`, which maps the same public proofs to AI social agents, Injective execution, the physical Frost Buddy extension, and the privacy-first evidence boundary. `submissionLinks` keeps the final review entry points together: the 60-second judge quickstart, GitHub repo, live `?demo` URL, public evidence API, single-agent proof API, fleet API, and wallet timeline API. `publicReadApis` repeats those four machine-openable product APIs as a small manifest with `GET`, `chainId 1439`, `readOnly: true`, `publicOnly: true`, a matching verification command, `judgeFocus`, and `expectedFields` for each path. `submissionChecklist` maps the contest submission requirements back to public proof: public GitHub + README, Injective integration, sub-3-minute demo script, pitch-deck notes, and read-only review APIs. `sourceControl` ties the public evidence API to the GitHub repository, `main` branch, current commit SHA, and commit URL. `recordingOrder[].evidenceFocus` tells a reviewer what to look for at each step: owner, single-agent proof/source anchor, `builderCode=pocket-earth`, mint summary, wallet timeline, and plaza grouping. `npm run verify:github` confirms the public GitHub repository, `origin/main`, and remote README/evidence files are the Injective submission; `npm run verify:integration-guide` checks the integration guide API table, runbook order, and npm script mappings; `npm run verify:source` confirms the evidence API source-control anchor matches the current checkout; `npm run verify:registry` reads the ERC-8004 Registry mint events for `agentId 43-47`; `npm run verify:agent-proof` opens every single-agent proof card; `npm run verify:pitch` checks the pitch notes, hardware boundary, and cited Raspberry Pi source; `npm run verify:judge` checks the one-page judge path and API `judgeRunbook`; `npm run verify:public-proof` treats the JSON as the judge-facing contract and guards its field shape, `judgeRunbook`, `publicReadApis` guidance, source anchor, recording-order focus, commands, public URLs, repo boundary, and secret/local-path leak surface.
...
The first call returns the public evidence package from the same `chain-proof-data.mjs` facts used by the verification suite. It is explicitly marked `network: testnet`, `chainId: 1439`, `readOnly: true`, and `publicOnly: true`, and includes `agents[]` proof rows where each agent directly carries owner, `builderCode`, registry, mint tx hash/block, mint tx link, Blockscout identity link, and `proofApi`; the follow-up verification commands/paths (`npm run verify:demo`, `npm run verify:duration`, `npm run verify:evidence`, `npm run verify:public-proof`, `npm run verify:public-apis`, `npm run verify:integration-guide`, `npm run verify:github`, `npm run verify:source`, `npm run verify:registry`, `npm run verify:agent-proof`, `npm run verify:pitch`, `npm run verify:judge`, `npm run verify:brief`, `npm run verify:review`, `npm run verify:review-links`, `npm run verify:recording-order`, `npm run verify:wallet`, `npm run verify:plaza-flow`, `npm run verify:nova-alignment`, `npm run verify:submission`, `npm run verify:injective`, `get-agent-proof`, `list-agents`, and `get-wallet-timeline`) plus `registryMintEvents` for ERC-8004 mint event rows, `registryMintSummary` for the agentId range / same-owner / zero-address mint / first-last block summary, `timeline` rows with the shared owner wallet in `from` and `expectedStatus: success`, `timelineSummary` for the owner/event-count/first-block/last-block/RPC-verification summary, `handshakeProof` for the decoded SocialHandshake meaning and its local bytecode/event verifier, `reviewBrief` for the one-page judge summary, `judgeRunbook` for the 60-second API-native review path, `reviewLinks` for the most important Blockscout pages, `reviewChecklist` for what each proof should establish, `competitionAlignment` for mapping the public proof to the Injective Nova story, `submissionLinks` for the final review entry points including `JUDGE-QUICKSTART.md`, `submissionChecklist` for mapping the contest's GitHub/README, Injective integration, demo video, pitch deck, and public API requirements to current proof, `publicReadApis` for the four judge-safe GET endpoints plus their verification commands, `judgeFocus`, and `expectedFields`, `sourceControl` for the current repository/branch/commit URL, a `recordingOrder` array for the recommended demo sequence, a `privacyBoundary` manifest that spells out what is on-chain versus kept off-chain, and a `plazaFlow` manifest that separates `public-plaza` chain discovery from the `agent-plaza` install loop. The plaza flow maps to `verify-plaza.mjs`, `verify-space-plaza.mjs`, and `verify-plaza-install.mjs`. The second call opens the #43 single-agent proof card. The third call reads `agentId 43-47` by `builderCode = pocket-earth`. The fourth call returns the RPC-backed wallet timeline above, including registration, SocialHandshake deployment, fleet registration, the real `agentId 43 <-> 44` handshake, and a `summary` with owner, event count, all-succeeded status, first/last block, and first/last timestamp. `npm run verify:duration` parses the demo script and keeps it within the 180-second submission limit; `npm run verify:judge` checks that `JUDGE-QUICKSTART.md` remains a public 60-second path through Blockscout, product APIs, local commands, and the API `judgeRunbook`; `npm run verify:public-apis` opens every `publicReadApis` entry through the product API and confirms the manifest, `chainId 1439`, `readOnly` / `publicOnly` flags, single-agent proof card, fleet readback, wallet timeline, reviewer guidance, source anchor, and leak guard; `npm run verify:integration-guide` checks that the integration guide API table, runbook order, and npm script mappings still match the product API; `npm run verify:wallet` checks the wallet timeline API as a focused proof of registration, deployment, fleet, and handshake transactions; `npm run verify:source` checks that the evidence API `sourceControl` field points to this repository, `main`, and the current commit; `npm run verify:registry` reads the ERC-8004 Registry `Transfer(0x0 -> owner, tokenId)` mint events for `agentId 43-47` and checks their registration transactions; `npm run verify:agent-proof` opens each single-agent proof card and checks identity, mint, wallet, and source anchors; `npm run verify:demo` is the pre-recording check for evidence API, public evidence contract, public read API manifest, GitHub repo boundary, source-control anchor, pitch notes, judge quickstart, review brief, judge runbook, fleet readback, registry mint events, registry mint summary, Blockscout links, recording order, plaza-flow grouping, competition alignment, submission links, and submission checklist; `npm run verify:evidence` is the fastest local smoke check for the JSON-only evidence package; `npm run verify:public-proof` checks that the judge-facing package keeps a stable field shape, public read API manifest and guidance, registry mint rows and summary, source-control anchor, only public review URLs, runnable verification commands, the Injective repo boundary, and no private env/local path/hash parameter leaks; `npm run verify:github` checks that `origin/main`, GitHub visibility, default branch, remote README, integration guide, evidence pack, and demo script still belong to the public `Pocket-Earth-Injective` submission; `npm run verify:pitch` checks that `PITCH-NOTES.md` keeps the deck story centered on Injective proof, carefully bounds Frost Buddy hardware claims, and cites the Raspberry Pi source; `npm run verify:brief` checks that the judge-facing `reviewBrief` still explains the Injective proof path, contest fit, and privacy line without local paths or secrets; `npm run verify:review` checks that the reviewer checklist still points at existing public links and local proof commands; `npm run verify:review-links` checks that the API-returned reviewer links, registry mint transaction links, registry mint identity links, and registry summary links are still complete, public-only, and reachable on Injective Blockscout; `npm run verify:recording-order` checks the exact recording order from Blockscout pages through the single-agent proof card, product API calls, and the plaza smoke; `npm run verify:plaza-flow` checks that `public-plaza` remains the chain-discovery flow and `agent-plaza` remains the marketplace/install flow; `npm run verify:nova-alignment` checks that the evidence package still maps Pocket Earth to AI social agents, Injective execution, the physical hardware extension, and privacy-first public proof without leaking local paths or secrets; `npm run verify:submission` checks that the final judge quickstart, GitHub, live demo, API entry points, and submission checklist still point to this Injective submission.
