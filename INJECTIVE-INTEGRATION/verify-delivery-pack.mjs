...
assertTrue('Frost Edge Node checklist mentions implemented physical driver', checklistByKey.get('frost-edge-node').evidence.includes('Whisplay/RGB/local-TTS/phone-mirror physical driver'))
assertTrue('Frost Edge Node checklist mentions optional transports', checklistByKey.get('frost-edge-node').evidence.includes('BLE/MQTT/serial transports stay optional'))
...
assertTrue('DEMO-SCRIPT separates public-plaza from agent-plaza', demoScript.includes('public-plaza` 讲链上身份发现') && demoScript.includes('agent-plaza` 讲创建、审核、发布、安装和运行'))
...
assertTrue('JUDGE-QUICKSTART names Chronicle revision 2', judgeQuickstart.includes('Daily Knowledge Chronicle Fast Check') && judgeQuickstart.includes('revision 2'))
assertTrue('JUDGE-QUICKSTART names knowledge proof API', judgeQuickstart.includes('/api/knowledge?tool=proof&recordHash=...'))
