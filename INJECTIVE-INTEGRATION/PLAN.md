...
> 总架构师方案。综合 RESEARCH.md 四份研究 + 用户提供的 ChatGPT 方案（Frost Passport + Agent Plaza + Nightly Chain Dispatch，7 功能）。
> 目标：**几小时内出 P0 Demo（本地/testnet 可跑）**。务实可执行，不为 web3 而 web3。
...
└──────────────────────────────────────────┬───────────────────────────────────────────────┘
                        ...
                        │ registerActionValidator('inj_agent_register'|'inj_tip'|'inj_bank_send') │
                        │ 校验：testnet-only · 金额/地址白名单 · 二次确认 flag                      │
                        └──────────────────┬─────────────────────┘
                                           │ 仅放行「动作意图」，私钥绝不进前端
                                           ▼
...
### 2.1 出门（注册身份）
`profile.ts → Taste Passport(脱敏) → 前端发 'inj_agent_register' 意图 → Boundary 校验(testnet-only) → /api/injective?tool=register → server 用 INJ_PRIVATE_KEY 签名 → AgentClient.register → IPFS card + 链上 NFT → 返回 {agentId,identityTuple,cardUri,scanUrl}` → 缓存到端侧（pe.inj.self.v1），`profileFingerprint` 当版本键，口味变了才重新 register/update。
...
| P0-2 | `server.mjs` 加 `handleInjective(req,res,url)` + `INJ_PK` env + 主路由 `/api/injective` + `/healthz` 加 inj 状态 | `curl /api/injective?tool=ping` 返回 reachable；`/healthz` 含 inj | viem + @injective/agent-sdk 装好 |
| P0-3 | `vite.config.ts` 加 `server.middlewares.use('/api/injective', ...)` | dev 模式同样打通（与 prod 一致） | P0-2 |
| P0-4 | `?tool=register` 跑通：服务端 `AgentClient.register({...,description=Passport})` | 返回真实 agentId/identityTuple/scanUrl，8004scan 能看到 | **用户私钥 + testnet INJ + (Pinata JWT 或自托管 card)** |
...
- **第 2 幕（0:30-1:10）注册上链**：点「让 FROST 出门」→ Boundary 弹「testnet-only，确认注册？」→ 服务端签名 → 几秒后弹出 `scanUrl`，打开 8004scan 看到这枚真实的 soulbound 身份 NFT + Agent Card（description=你的口味）。旁白：「这是一枚**不可转让的身份 NFT**，链上从此有了你的口味分身。」
