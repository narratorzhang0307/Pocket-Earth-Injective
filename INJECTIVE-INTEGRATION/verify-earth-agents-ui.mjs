...
assert.match(hub, /<PublicEarthPage/)
...
assert.match(agents, /aria-expanded=\{factRelayOpen\}/)
assert.match(agents, /data-testid="fact-relay-trigger"/)
assert.match(agents, /data-testid="fact-relay-panel"/)
assert.match(agents, /setFactRelayOpen\(\(open\) => !open\)/)
assert.doesNotMatch(agents, /verification-trigger-/)
assert.equal((data.match(/guardrail:/g) || []).length, 7)
