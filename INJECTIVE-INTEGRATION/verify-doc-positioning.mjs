...
const progress = readFileSync(resolve(projectRoot, 'INJECTIVE-INTEGRATION/PROGRESS.md'), 'utf8')
const plan = readFileSync(resolve(projectRoot, 'INJECTIVE-INTEGRATION/PLAN.md'), 'utf8')
const research = readFileSync(resolve(projectRoot, 'INJECTIVE-INTEGRATION/RESEARCH.md'), 'utf8')
...
assertTrue('PLAN declares current evidence source before historical plan text', plan.includes('当前事实以 `README.md`、`INJECTIVE-INTEGRATION/CHAIN-EVIDENCE.md`、`INJECTIVE-INTEGRATION/JUDGE-QUICKSTART.md` 和五条只读 API 为准'))
assertTrue('PLAN uses data URI Agent Card wording', plan.includes('data URI Agent Card') && !plan.includes('Agent Card `description`（IPFS）') && !plan.includes('IPFS Agent Card'))
assertTrue('RESEARCH declares current evidence source before SDK background', research.includes('当前事实源：`README.md`、`INJECTIVE-INTEGRATION/CHAIN-EVIDENCE.md`、`INJECTIVE-INTEGRATION/JUDGE-QUICKSTART.md` 和五条只读 API'))
assertTrue('RESEARCH uses data URI Agent Card wording', research.includes('data:application/json;base64') && !research.includes('IPFS Agent Card'))
