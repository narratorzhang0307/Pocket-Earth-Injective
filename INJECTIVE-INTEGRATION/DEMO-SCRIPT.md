...
- **录制前 smoke**：先跑 `npm run verify:demo`，确认公开证据包、`builderCode=pocket-earth` 读回 `agentId 43–47`、Blockscout 展示链接都正常；再跑 `npm run verify:review` 和 `npm run verify:review-links`，确认 `reviewChecklist` 仍指向公开链接/本地复验命令，产品 API 返回的评审主链接也都能打开；最后跑 `npm run verify:plaza`，自动启动页面并按证据包里的 `plazaFlow` 分别检查 public-plaza 读链上 agent、agent-plaza 页面和「咖啡地图」安装闭环。
