...
- 交易时间线：`/api/injective?tool=get-chain-evidence` 的公开 `timeline` 会先给每笔关键交易标出同一 `from` 钱包和 `expectedStatus: success`；`verify-chain-timeline.mjs` 会再通过 Injective testnet JSON-RPC 直接读取注册、合约部署、fleet 注册和真实握手的 transaction / receipt / block，核验区块号、UTC 时间戳、from/to 与成功状态，作为钱包页录屏的机器可复验证据。
