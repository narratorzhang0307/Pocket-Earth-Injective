...
| 6-8 | 三个 Tab、中间地球、PHOTOS 三视图 | `userMarks` / `planets` / `geoStickers` store，Mapbox `source.setData()`，照片缩略图/高清分层 | `npm run verify:demo` |
...
## 11. 技术深挖：Agent Plaza 与 Frost Buddy
...
| Frost Buddy | `hardware/frost-buddy/` 用 JSONL 事件桥连接 Raspberry Pi / BLE / TTS | 只消费公开 chain_dispatch 和 music-agent 事件，不接触私钥、画像原文、`bytes32` 哈希原文 |
...
- Raspberry Pi 只作为可信原型平台；硬件细节以 `PITCH-NOTES.md` 的来源边界为准，不写成硬件市场规模预测。
