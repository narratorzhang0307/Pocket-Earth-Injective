...
| GET | `?tool=get-chain-evidence` | 公开证据包：testnet chainId 1439、agentId 43–47、钱包、合约、时间线交易、Blockscout 链接、`reviewLinks`、`reviewChecklist`、`competitionAlignment`、`submissionLinks`、`recordingOrder`、`privacyBoundary`、`plazaFlow` 与录屏/复验命令 | 否 |
...
# 7. 验证产品 API 返回给评审的 Blockscout 主链接仍可打开
npm run verify:review-links

# 8. 验证证据包里的录屏顺序可从 Blockscout 一路走到产品 API
npm run verify:recording-order

# 9. 验证 public-plaza / agent-plaza 演示分组未漂移
npm run verify:plaza-flow

# 10. 验证公开证据包和 Injective 新星计划叙事对齐
npm run verify:nova-alignment

# 11. 验证提交入口不误指旧仓库，并固定 demo/API 路径
npm run verify:submission

# 12. 一键验证 Injective 链上证据（只读，不需要私钥）
npm run verify:injective

# 13. 录屏前验证 public-plaza / agent-plaza 前端闭环（会自动启动/关闭本地 Vite）
npm run verify:plaza

# 14. 验证 Frost Buddy 硬件桥事件契约（只读，无硬件也能跑）
