...
# 10. 验证评审简报仍能一页讲清 Injective 价值和复验路径
npm run verify:brief

# 11. 验证评审清单仍指向公开链接和本地复验命令
npm run verify:review

# 12. 验证产品 API 返回给评审的 Blockscout 主链接仍可打开
npm run verify:review-links

# 13. 验证证据包里的录屏顺序可从 Blockscout 一路走到产品 API
npm run verify:recording-order

# 14. 验证 public-plaza / agent-plaza 演示分组未漂移
npm run verify:plaza-flow

# 15. 验证公开证据包和 Injective 新星计划叙事对齐
npm run verify:nova-alignment

# 16. 验证提交入口不误指旧仓库，并固定 demo/API 路径
npm run verify:submission

# 17. 一键验证 Injective 链上证据（只读，不需要私钥）
npm run verify:injective

# 18. 录屏前验证 public-plaza / agent-plaza 前端闭环（会自动启动/关闭本地 Vite）
npm run verify:plaza

# 19. 验证 Frost Buddy 硬件桥事件契约（只读，无硬件也能跑）
