...
| Frost Buddy | music-agent 和链上见闻可以被实体 Frost 播报 | `hardware/frost-buddy/` 只消费公开 JSONL 事件，Pi adapter 只输出 `state/tts/display` 动作，不接触私钥或画像原文 |
...
PPT 第 5 页展示的是 Pocket Earth 的最小闭环：一个现实碎片先被端侧脱敏，再被钉回地球；长期使用沉淀成公开 Taste Passport，Frost 再带着脱敏名片去 public-plaza 读取 Injective 链上 agent。这个过程的关键不是“写了很多功能”，而是每一步都有明确的数据边界和复验入口。

| 阶段 | 工程动作 | 数据边界 | 复验入口 |
|---|---|---|---|
| 1. 票根 / 截图 / 书影音输入 | `JOT` 或对应 agent 接收碎片，走 vision / text extraction 与 domain routing | 原图、票据、证件号、手机号先端侧处理，不进入链上证据包 | `README.md` 的“Pocket Earth 是什么”、`verify:positioning` |
| 2. 端侧脱敏与结构化 | `visionRead` + REDACT 正则先擦敏感字段，再产出结构化行程、书影乐或心情候选 | 只保留必要字段；原始照片、票据、心情原文不公开 | `docs/技术难点与解决方案.md`、`privacyBoundary` |
| 3. 钉回地球 | `markPlace` 写入 `userMarks` / `planets` / `geoStickers`，Mapbox `source.setData()` 即时重绘 | 地球是本地索引，不等于把精确生活轨迹上链 | `verify:demo`、`verify:positioning` |
| 4. 形成公开名片 | 长期画像只导出 top-K 公开标签，生成 Taste Passport 和 `profileHash` 候选 | 公开的是标签和版本指纹，不是完整书单、照片或心情 | `verify:public-proof`、`buildTastePassport()` |
| 5. Frost 出门社交 | `public-plaza` 按 `builderCode=pocket-earth` 读回 `agentId 43-47`，计算公开标签重叠并钉 agent 点 | 只读 Injective testnet；不需要私钥，不写链 | `list-agents&builderCode=pocket-earth`、`verify:plaza-flow` |
| 6. 链上见闻回到产品 | Nightly Chain Dispatch 把相遇、相似度和推荐写成人话；SocialHandshake 只记录 agentId、承诺哈希、score、timestamp | 链上是身份和回执，私人记忆仍留在端侧/服务端 | `get-chain-evidence`、`get-wallet-timeline`、`verify:wallet` |

这条线把 PPT 里的“随手记一笔，看它怎么长成你的地球”落成了工程闭环：现实碎片先进入 Pocket Earth 的空间知识库，再经过 frost-agent harness 变成可公开的 agent 名片，最后由 Injective 见证公开身份、时间顺序和社交回执。
...
| Frost Buddy | Raspberry Pi / BLE / TTS 事件桥，让 music-agent 与链上见闻被实体 Frost 播报；Pi adapter 输出 `state/tts/display` 三类设备动作 | 只消费公开事件，不接触私钥、画像原文或精确坐标 |
...
| Frost Buddy 硬件事件桥，支持 music-agent + Injective 链上见闻 -> JSONL -> Raspberry Pi skill router -> Pi adapter `state/tts/display` -> BLE / TTS / display | `hardware/frost-buddy/` | 已实现 |
...
| Frost Edge Node | `hardware/frost-buddy/` 用 JSONL 事件桥连接 Raspberry Pi / BLE / TTS；Pi adapter 只产出 `state/tts/display` 动作 | 只消费公开 chain_dispatch 和 music-agent 事件，不接触私钥、画像原文、`bytes32` 哈希原文 |
...
这条分支把 PPT 第 33 页的“实体节点，不是重资本硬件路线”落成了工程结构：Pocket Earth 主线只负责生成公开事件；Pi adapter 只负责把公开事件翻译成设备动作；Whisplay 小屏幕、RGB LED、本地 MiniMax / 离线 TTS 和手机镜像 driver 位于 adapter 之后，已经完成并可替换、可删除、可单独测试。`hardwareBridge.roadmapBoundary` 会把“事件桥、action 合同与当前物理 driver 已完成，BLE / MQTT / serial 仍是可选 transport”的边界暴露到公开证据 API；`frost_pi_event_adapter_smoke.py` 验证 action 合同和私密哈希拦截，`frost_pi_device_driver_smoke.py` 验证 240×280 RGB565 物理渲染，真机 `frost_pi_live_preflight.py --strict` 验证 Wi-Fi、systemd、Whisplay、扬声器、镜像和 cursor。
...
`marketLandscapeBoundary` 把 PPT 第 32-35 页的市场判断压成公开 API 字段，避免商业叙事只停留在口号里。它和 `hardwareBridge.marketBoundary` 的职责不同：前者解释商业路径和三条反面路径，后者约束 Frost Edge Node 不被写成短期硬件收入。

| 字段 | 含义 | 核验方式 |
|---|---|---|
| `commercialFlywheel` | 固定“长期使用 -> 可信画像 -> Agent 市场 -> 安装/调用/评价/可选付费回执 -> 更好体验”的飞轮 | `/api/injective?tool=get-chain-evidence`、`npm run verify:public-proof` |
| `preferredPath` | 固定 `Agent Plaza platform path`，并要求 `agent-plaza` 已能展示 manifest review、`INSTALL`、My Agents 回流和 `willEmit` dry-run | `npm run verify:plaza-flow`、`npm run verify:pitch` |
| `rejectedPaths` | 明确排除纯社交变现、代币优先、硬件收入优先三条路径 | `npm run verify:public-apis`、`npm run verify:evidence` |
| `differentiation` | 端侧隐私、真实坐标知识库、ERC-8004 公开身份、平台抽成兼容的 Agent Plaza 四件事同时成立 | `npm run verify:delivery` |

这个字段让“平台模式有成熟参照”保持克制：Roblox、Apple、Steam 只是成熟创作者/分发平台的模式参照，不被写成 Pocket Earth 的收入预测；Injective 的角色仍是身份、版本、回执和未来结算入口。

### PPT 第 34 页的反面坐标如何进入仓库

最终整合版 PPT 把市场风险说得很直接：纯社交、代币优先和消费级 AI 硬件都不能直接当成 Pocket Earth 的主线。Markdown 不把这些外部案例写成可复验收入预测，只把它们转译成产品边界。

| PPT 反面坐标 | 在文档中的处理 | Pocket Earth 的选择 |
|---|---|---|
| `friend.tech` / Lens / Farcaster | 只作为“纯社交变现不够稳”的风险提示，不把陌生人社交写成核心商业来源 | `public-plaza` 负责链上社交发现，但价值回到长期空间记忆和可信画像 |
| Virtuals 等 agent 代币路径 | 只作为“代币发行和交易费不能证明 agent 有用”的风险提示 | Injective 先承担身份、版本、回执和公开证明；可选结算放到 Agent Plaza 后续阶段 |
| Humane AI Pin / Rabbit r1 等消费级 AI 硬件 | 只作为“硬件收入优先风险高”的风险提示；仓库不使用无法复验的留存率或失败归因数字 | Frost Edge Node 先做 Raspberry Pi / BLE / TTS 开发套件和体验差异化，不写成量产收入承诺 |

### 三条商业边界

| 边界 | 为什么不作为主线 | Pocket Earth 的主线 |
|---|---|---|
| 纯社交变现 | 只靠陌生人社交很难证明长期价值，链上握手也不能替代真实使用 | public-plaza 只负责链上社交发现，价值回到空间知识库和长期画像 |
| 代币优先 | 代币发行和交易费不能证明 agent 真的有用，也会压过产品本身 | Injective 先承担身份、版本、回执、公开证明；可选结算放到 Agent Plaza 后续阶段 |
| 硬件收入优先 | 消费级 AI 硬件风险高，量产、售后和渠道会吞掉早期团队精力 | Frost Edge Node 先作为 Raspberry Pi / BLE / TTS 开发套件和体验差异化 |

因此 Agent Plaza 是商业路径的中心：开发者发布 `manifest / schema / permissions`，平台用 `reviewManifest` 和 `toManifest` 做权限与空间逻辑审核，用户安装到 My Agents 后再运行；未来安装、调用、评价、可选付费都会成为 Profile Confidence 的外部回执。

## 14. 技术深挖：用户痛点与对症解决

PPT 第 36-38 页讲的是产品为什么成立：个人记忆缺一根能把一切串起来、记得住、还护隐私的轴。Pocket Earth 把这根轴定义为真实地理坐标，再用端云双脑、skill 沉淀和 Injective 公共见证层把它变成可运行系统。

| 用户痛点 | Pocket Earth 的解法 | 工程证据 |
|---|---|---|
| 散落各处 | 书、影、乐、照片、行程、心情六类碎片统一钉回真实坐标，不再散在多个 App 里 | `userMarks`、`planets`、`geoStickers`、Mapbox `source.setData()` |
| 按时间记不牢 | 用“在哪”当记忆主键；用户想不起日期或标题时，仍能从地点、路线和地球标记找回 | `markPlace`、地球入口、agent 标记、mood trace |
| 记完就沉底 | Frost 会把记录重新唤起：public-plaza 的 Nightly Chain Dispatch、music-agent、travel-agent 和 council 都把碎片变成可回看的叙事 | `PublicPlazaPage.tsx`、`open-dj-director`、`radio-24h-director`、`src/app/council/` |
| 工具不懂你 | 跨会话长期画像、fingerprint 缓存和反思回流让 Frost 只在口味实质变化时刷新，不重复推你已经看过的内容 | `profile.ts`、`memoryRouter.ts`、`profileFingerprint()` |
| 隐私不敢交 | 原图和票据先在端侧脱敏；链上只放身份、版本、回执和公开证明，不放原始书影音、照片、心情或精确坐标 | `visionRead`、`REDACT`、`privacyBoundary`、`publicOnly: true` |
| 整理太费劲 | 端侧 Selector 管挑找，云端 Brain 管表达；运行时 skill 负责识图、落点、检索、记忆和反思，新增 agent 尽量复用 skill | `edgeSafe`、`intentRegistry`、`registerActionValidator`、`src/app/lib/skills/` |

### 创新影响如何落到架构

| PPT 结论 | 仓库里的落点 |
|---|---|
| 地球作索引 | 真实坐标不是装饰图层，而是六类对象的共同索引；所有 agent 产出最终回到同一颗地球。 |
| 端云双脑 + 链上身份 | 端侧负责挑选、检索、脱敏；云端负责表达；Injective 负责 ERC-8004 身份、钱包时间线、SocialHandshake 和未来 Profile Checkpoint。 |
| 能力沉淀 skill | skill 是可复用能力层，agent 只是调用组合；这让“新增一个空间 agent”不需要复制一整套工具链。 |
| 越用越懂你 | 长期画像、fingerprint 缓存、回流和反思构成 Proof of Memory 的本地来源层；Profile Chain 只见证公开画像版本。 |
| 已上线且有数据底 | README 固定展示 2124 部影、1055 本书、619 位艺人、536 张照片，说明这不是空白演示壳。 |
| 护隐私 | 原始记忆不上链、敏感数据不进公开 API、Frost Edge Node 只消费公开 JSONL 事件。 |
