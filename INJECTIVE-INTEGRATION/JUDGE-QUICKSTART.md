...
7. Open the Frost Edge Node proof card: `/api/injective?tool=get-hardware-bridge-proof`, starting from `hardwareBridge.chainDispatch`, `piRouter.skills`, `piAdapter.actions`, `hardwareBridge.marketBoundary`, `hardwareBridge.roadmapBoundary`, and `privacyBoundary.hardware`
