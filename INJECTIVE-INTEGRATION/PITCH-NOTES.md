...
| 4 | Agent 内核 | 主 Frost 只做委派，专门子 Agent 在隔离上下文中工作，并由 harness 保持失败可降级。Frost 不是贴在产品上的吉祥物，而是跨 App、身份卡与实体设备保持一致的 Agent 角色。 | 用 CEO 委派解释可组合性，不把 Agent 说成智能合约；指出统一像素轮廓。 |
...
- Frost 的品牌规则是“统一轮廓，个人细节”：App、Public Earth 身份卡、PPT 与 Frost Edge Node 使用同一像素轮廓、名称和基础性格；未来个性化只读取用户主动公开的脱敏标签，不读取私人记忆原文。现场只说“Frost 不是吉祥物，是产品本身——一个努力理解你、替你整理人生的 Agent”，不引用其他 NFT 项目，也不把文学灵感说成授权改编。
...
Frost 同时承担产品入口与品牌识别：**统一的方头像素轮廓提供识别度，用户主动公开的脱敏标签提供个体差异。** 当前已实现的是 App、身份卡与设备侧的统一基础形象；“由公开标签确定性生成跨端配色、表情和饰件”仍是赛后路线，不在决赛中说成已完成。
