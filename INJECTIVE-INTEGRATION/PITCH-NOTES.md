...
每个 FROST 可以拥有一张可视化 Agent 身份卡：卡面是角色形象，卡背是 agentId、能力版本、知识贡献和服务历史。卡牌属性只能由可验证行为演进，不能靠购买属性；Frost Edge Node 读取同一身份后加载对应头像、颜色、声音与技能包。卡、软件与硬件是同一个 Agent 的三个表面，硬件是身份的物理登录终端，而不是人为制造稀缺的收藏品。
