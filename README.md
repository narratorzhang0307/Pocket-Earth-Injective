...
- **产品闭环**：`/api/injective` 读取链上 agent，`public-plaza` 展示相似 agent，地球出现蓝紫色链上标记，Nightly Chain Dispatch 把链上见闻翻译成人话。
- **一键验链**：本地运行 `npm run verify:injective`，会只读核验 `agentId 43`、`builderCode: pocket-earth`、Blockscout 页面和真实握手事件。
