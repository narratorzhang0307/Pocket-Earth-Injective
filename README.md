...
- **产品闭环**：`/api/injective?tool=list-agents&builderCode=pocket-earth` 从 Injective 链上按 builderCode 读回 `agentId 43–47`，`/api/injective?tool=get-chain-evidence` 输出 testnet `chainId 1439` 的只读公开证据包，并内含 `reviewLinks`、`reviewChecklist` 与 `competitionAlignment` 帮评审按步骤复验和对照比赛主题；`/api/injective?tool=get-wallet-timeline` 从 RPC 读回钱包交易时间线，`public-plaza` 展示相似 agent，地球出现蓝紫色链上标记，Nightly Chain Dispatch 把链上见闻翻译成人话。
...
- **一键验链**：本地运行 `npm run verify:demo` 可在录屏前快速确认公开证据包、`builderCode` 读回 `agentId 43–47`、Blockscout 展示链接、录屏顺序、plaza 分组和比赛对齐清单都正常；`npm run verify:review` 会检查评审清单仍指向公开链接和本地复验命令；`npm run verify:review-links` 会从产品 API 读取评审主链接并实际打开 Injective Blockscout；`npm run verify:recording-order` 会按证据包里的录屏顺序验证 #43、钱包页、公开证据 API、fleet API、钱包时间线 API 和 plaza smoke；`npm run verify:plaza-flow` 会固定 public-plaza = 链上社交发现、agent-plaza = agent 市集/安装闭环；`npm run verify:nova-alignment` 会固定 AI 社交、Injective 链上执行层、Agent × 物理世界硬件延展与隐私公开证据的对应关系；运行 `npm run verify:injective` 会只读核验 `agentId 43–47`、公开 data URI 名片结构、`builderCode: pocket-earth`、`/api/injective` 读链路（ping/list/status/reputation/chain-evidence/wallet-timeline）、写链 dry-run 边界、ERC-8004 registry mint 事件、钱包证据链、SocialHandshake 部署交易/源码字节码、Demo/README 的 Blockscout 链接、带非零名片哈希的真实握手 hash derivation + calldata + event，以及 Frost Buddy 硬件桥不会泄露私钥或画像原文。
