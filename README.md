...
- **一键验链**：本地运行 `npm run verify:demo` 可在录屏前快速确认公开证据包、`builderCode` 读回 `agentId 43–47`、Blockscout 展示链接都正常；`npm run verify:review` 会检查评审清单仍指向公开链接和本地复验命令；`npm run verify:review-links` 会从产品 API 读取评审主链接并实际打开 Injective Blockscout；运行 `npm run verify:injective` 会只读核验 `agentId 43–47`、公开 data URI 名片结构、`builderCode: pocket-earth`、`/api/injective` 读链路（ping/list/status/reputation/chain-evidence/wallet-timeline）、写链 dry-run 边界、ERC-8004 registry mint 事件、钱包证据链、SocialHandshake 部署交易/源码字节码、Demo/README 的 Blockscout 链接、带非零名片哈希的真实握手 hash derivation + calldata + event，以及 Frost Buddy 硬件桥不会泄露私钥或画像原文。
