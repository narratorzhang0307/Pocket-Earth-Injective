...
- **产品闭环**：`/api/injective?tool=list-agents&builderCode=pocket-earth` 从 Injective 链上按 builderCode 读回 `agentId 43–47`，`/api/injective?tool=get-wallet-timeline` 从 RPC 读回钱包交易时间线，`public-plaza` 展示相似 agent，地球出现蓝紫色链上标记，Nightly Chain Dispatch 把链上见闻翻译成人话。
