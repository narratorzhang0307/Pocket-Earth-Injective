...
- **产品闭环**：`/api/injective` 读取链上 agent（当前可复验 `agentId 43–47`），`public-plaza` 展示相似 agent，地球出现蓝紫色链上标记，Nightly Chain Dispatch 把链上见闻翻译成人话。
- **一键验链**：本地运行 `npm run verify:injective`，会只读核验 `agentId 43–47`、`builderCode: pocket-earth`、`/api/injective` 读链路（ping/status/reputation/list）、写链 dry-run 边界、ERC-8004 registry mint 事件、钱包证据链、SocialHandshake 部署交易/源码字节码、Demo/README 的 Blockscout 链接，以及带非零名片哈希的真实握手 hash derivation + calldata + event。
